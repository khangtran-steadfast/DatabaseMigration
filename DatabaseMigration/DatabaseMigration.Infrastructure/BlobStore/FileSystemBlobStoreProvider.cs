...
namespace DatabaseMigration.Infrastructure.BlobStore
{
    /// <summary>
    ...
    public class FileSystemBlobStoreProvider : BlobStoreProvider
    {
        ...
        public FileSystemBlobStoreProvider()
        {
            ...
            {
                throw new MigrationException(MigrationExceptionCodes.SERVICE_ERROR_APP_SETTING, APP_SETTING_ROOT_DIR, _rootPath);
            }
        ...
        private string UriToPath(string key, bool checkIfFileExists, bool create)
        {
            ...
            { throw new MigrationException(MigrationExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "null or empty", key); }

            string[] parts = key.Split(':');
            if (parts.Length != 4)
            { throw new MigrationException(MigrationExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "4 parts expected", key); }

            string category = parts[0];
            string ts = parts[1];
            if (ts.Length != 10)
            { throw new MigrationException(MigrationExceptionCodes.BLOB_STORE_PROVIDER_ERROR_INVALID_KEY, "timestamp format not 'yyyyMMddHH'", key); }
            ...
            if (!dir.Exists)
            {
                ...
                {
                    throw new MigrationException(MigrationExceptionCodes.BLOB_STORE_PROVIDER_ERROR_NOT_FOUND, key);
                }
            }
        ...
        public override string PutBlob(string category, string name, Stream contents)
        {
            ...
            if (file.Exists)
            { throw new MigrationException(MigrationExceptionCodes.BLOB_STORE_PROVIDER_ERROR_EXISTS, file.FullName); }
